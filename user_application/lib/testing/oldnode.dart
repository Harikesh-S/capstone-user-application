...
class NodePage extends StatefulWidget {
  const NodePage({Key? key, required this.socket}) : super(key: key);
  final Socket socket;
  @override
    ...
    if (!isListening) {
      widget.socket.listen(
        (List<int> event) async {
          SecretKey secretKey = SecretKey(utf8.encode("abcdefghijklmnop"));
    ...
    return Scaffold(
      ...
      body: Center(
        child: Column(
          ...
          children: <Widget>[
            const Text("Connected to node"),
            ElevatedButton(
              onPressed: () async {
                final nonce = AesGcm.with128bits().newNonce();
                final secretKey = SecretKey(utf8.encode("abcdefghijklmnop"));
